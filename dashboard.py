...
body.dark tr.sel td{background:#1e3152!important}
...
<script>
const HOBOKEN=[[40.7340,-74.0407],[40.7590,-74.0380],[40.7590,-74.0225],[40.7490,-74.0235],[40.7355,-74.0275]];
...
function render(d){
  ...
  for(const f of d.flights){
    ...
    const w=f.confidence==='high'?2.5:1.5,op=f.confidence==='high'?.8:.35;
    const pl=L.polyline(ll,{color:col,weight:w,opacity:op,dashArray:f.confidence==='low'?'5,5':null})
     ...
     .addTo(map);
    polylines[f.flight_id]=pl;
    polyStyles[f.flight_id]={color:col,weight:w,opacity:op};
    (function(fid){pl.on('click',function(e){L.DomEvent.stopPropagation(e);selectFlight(fid,true);});})(f.flight_id);
  }
  let selFid=null;
  function selectFlight(fid,fromMap){
    if(selFid===fid){clearSel();return;}
    clearSel();
    selFid=fid;
    const pl=polylines[fid];
    if(pl){
  ...
  function clearSel(){
    if(!selFid)return;
    for(const[id,p]of Object.entries(polylines)){const s=polyStyles[id];p.setStyle({color:s.color,weight:s.weight,opacity:s.opacity});}
  ...
  if(d.flights.length){
    ...
    document.getElementById('tbody').innerHTML=d.flights.map(f=>{
      ...
      return`<tr data-fid="${f.flight_id}">
    ...
    document.getElementById('tbody').addEventListener('click',function(e){
      ...
      if(row&&row.dataset.fid)selectFlight(row.dataset.fid,false);
    });
  }else{
